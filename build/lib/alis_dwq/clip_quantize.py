...
def _plan_ffn_perms(student, source, bases, sc_of, mx, overrides=()):
    ...
    for parent, leaves in sorted(parents.items()):
        ...
        if ok:
            ...
            # plan at the EFFECTIVE group size: a :gsG override changes the
            # boundaries the outlier dealing must target
            ovr = match_override(overrides, down) if overrides else None
            if ovr is not None and ovr[1] is not None:
                gs = ovr[1]
                ok = ok and ffn % gs == 0
        if not ok:
...
def main():
    ...
    for spec, one in zip(a.bits_override, overrides):
        # a typo'd name or an out-of-range layer band would otherwise no-op
        # silently and ship the old precision (no silent caps)
        if not any(match_override([one], b) for b in bases):
            raise SystemExit(f"[clip] --bits-override {spec!r} matches no "
                             "quantized tensor in the student — check the "
                             "name/layer range (nothing would be promoted)")
    ...
    if a.permute_ffn:
        ...
        perms = _plan_ffn_perms(student, source, bases,
                                lambda b: student[b + ".scales"], mx, overrides)
    ...
    def process(base):
        ...
        # hoisted so source-precision guards compare against the override
        # TARGET bits, not the student's pre-override packing
                ...
                if s_bi is None:
                    ...
                    if not a.allow_lattice_source:
                        ...
                        if (str(s_sc.dtype) == "mlx.core.uint8" and s_sc.shape[-1] * 16 == s_in
                            ...
                            if (ov[0] if ov else wq.shape[-1] * 32 // s_in) >= 4:
                                reason = "source precision (nvfp4~4b) does not exceed target — pass-through"
                            else:
                    ...
                    act = [(sb, sg) for sb, sg in cands
                           if sb > (ov[0] if ov
                                    else wq.shape[-1] * 32 // (s_sc.shape[-1] * sg))
                    ...
                    if len(act) == 1:
                        ...
                        reason = "source precision does not exceed target — pass-through"
                    else:
