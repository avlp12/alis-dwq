...
# Pre-parse the flags the provenance gates depend on with argparse itself,
# NOT an argv string scan: upstream's parser (allow_abbrev, '=' forms, -m
# alias, int coercion, last-occurrence-wins) accepts spellings like
# '--target-d X' and '--se 0' that a literal scan misses — and a missed
# --target-dir silently disables target verification, a missed seed 0
# silently desyncs every replay.
_pre = argparse.ArgumentParser(add_help=False)
_pre.add_argument("--model", "-m")
_pre.add_argument("--seed", type=int, default=123)
_pre.add_argument("--target-dir")
...
provenance.TARGET_DIR = _PRE.target_dir
...
def _wired_compute(model, *a, **k):
    ...
    if provenance._rank() == 0:
        manifest = provenance.build_manifest(
            ...
            extra={"model": _PRE.model, "loader": _loader_info})
        # sanity BEFORE the manifest exists: a failed dump must not leave a
        # manifest behind, or a retry (upstream's any-glob has_targets skips
        # the re-dump) would verify clean against rejected files
