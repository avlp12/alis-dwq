...
def layerwise_dwq_quantize(
    ...
    if not seed:
        # defense in depth behind run.py's pre-parse guard: iterate_batches
        # only reseeds truthy seeds, so 0/None desyncs the batch order from
        # the target dump and between per-round validation replays
        raise SystemExit("[alis-dwq] refusing to train with seed=0/None — "
                         "batch order would not replay; use a non-zero --seed")
