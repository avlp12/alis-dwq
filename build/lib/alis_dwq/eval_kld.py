...
PKG_DATA = Path(__file__).resolve().parent / "data"  # shipped in the wheel
...
def get_tokens(tokenizer, n=3072):
    ...
    if missing:
        raise SystemExit(
            ...
            "these are local files, not part of the repo (see "
            "https://github.com/avlp12/alis-dwq/blob/main/data/README.md "
...
# tolerate markdown emphasis/backticks after the colon and well-formed
# thousands grouping ('ANSWER: **52,432**') — two of the shipped answers are
# >= 5 digits and ZH outputs commonly group digits; a formatting-only FAIL
# would be a false ship-block
_ANSWER_RE = re.compile(r"ANSWER\s*:\s*[*_` ]*(-?\d+(?:,\d{3})*)", re.IGNORECASE)
...
def extract_answer(text):
    ...
    a long thinking chain cannot false-fail the run by itself — the measured
...
def main():
    ...
    ap.add_argument("--reason-probe", nargs="?",
                    const=str(PKG_DATA / "reason_probe.jsonl"),
                    default=None, metavar="FILE",
                    help="generative long-form reasoning gate: jsonl problems "
                         "({id, prompt, answer}), graded on the final "
                         "'ANSWER: <int>' line (default: the packaged "
                         "alis_dwq/data/reason_probe.jsonl)")
