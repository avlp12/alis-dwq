...
Measured (2026-07-15). Cross-family, the Bonsai phenomenon reproduces at **56×** (FP16 Qwen3.6-27B selfKL 0.162 vs its ternary build 0.0029 — see the [audit](examples/bonsai-27b-audit/README.md)). In-family on our GLM-5.2 quants the *direction* holds but the magnitude is mild — a clean monotone gradient, lower-bit student = more KV-tolerant: **2.3 bpw 0.079 / flip 8.9% → 2.56 bpw 0.113 / 13.0% → 3.5 bpw 0.143 / 12.1%** (overall selfKL, kv 4-bit gs64). Two readings: (a) PTQ+DWQ students do *not* inherit Bonsai-class KV immunity — consistent with the Tier-2 verdict that their tolerance is a training outcome, not a container property; (b) ZH is the most KV-sensitive slice on every build (0.122/0.186/0.245 vs EN 0.074/0.096/0.105) — quantized-KV serving hits the language slices hardest exactly where weight quantization already does.
