...
`82db86d9`, first 8 hex chars, for wikitext/code/zh respectively — the only
run so far with recorded corpus hashes; earlier case studies did not record
theirs, so hash your own files the same way).
...
The `eval_kld --reason-probe` problem set is different — it ships *inside*
the package (`alis_dwq/data/reason_probe.jsonl`, so wheel installs get it
too): original problems with brute-force-verified integer answers. Keep
those problems out of `gen_calib` seed corpora (calibration/eval
disjointness applies to the probe too).
