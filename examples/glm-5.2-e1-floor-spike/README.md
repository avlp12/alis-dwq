...
**Final: DWQ on E1r-s11** (4.5-bpw teacher, same recipe as Part 1) — valid 0.4925 → 0.3234 (12 accepted, 1 late revert), and every held-out metric beat the Part 1 champion: **wikitext 3.8985** (A45: 4.0364, −3.4%, non-overlapping CIs), code 2.1052 (−0.8%), tulu 3.608 (−1.4%), KL-vs-ref 0.448 (−12%), ZH-slice KL 0.686 (vs 0.793). Raw → shipped: **4.711 → 3.899 (−17.2%)**, with the clip contribution surviving DWQ almost fully — the two passes are orthogonal in practice, not just in principle. The `--loop-probe` degeneration gate (first field use) came back clean: distinct-4gram 0.90–0.98, no cycles on any slice. Shipped as the repo's 2.3 bpw `main`.
